...
class TestWhereaboutDAO: BaseTest() {
    ...
    fun testUpdateUserWhereabout() {
        TODO("Need to finish this once Users are complete. Uncomment the code below once it is complete")
        val whereaboutDAO = WhereaboutDAO()
//        val userDAO = UserDAO()
...
//        assertEquals("Other user's current zone should remain unchanged", WhereaboutTestResources.GROUP_1_ZONE_2_ID, user2.currentZoneID)
//
//
...
//        assertNull("User's current zone should be changed to null",  user1.currentZoneID)
...
//        assertEquals("Since userID was invalid, nothing should be affected",  user1.currentZoneID)
//        val user2Invalid = userDAO.getUser(UserTestResources.GROUP_1_USER_2_ID)
//        assertEquals("Since userID was invalid, nothing should be affected", WhereaboutTestResources.GROUP_1_ZONE_2_ID, user2.currentZoneID)
    }
}